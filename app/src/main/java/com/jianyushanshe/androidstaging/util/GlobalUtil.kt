 ...
 */
val sharedPreferences: SharedPreferences = MyApp.context.getSharedPreferences(
    appPackage + "_preferences",
 ...
 * 获取应用程序包名
 */
val appPackage: String get() = BuildConfig.APPLICATION_ID

/**
 * 获取应用名称
 */
val appName: String get() = MyApp.context.resources.getString(MyApp.context.applicationInfo.labelRes)
...
fun getStringResId(resId: Int): String {
    return MyApp.context.resources.getString(resId)
}
...
fun getDimension(resId: Int): Int {
    return MyApp.context.resources.getDimensionPixelOffset(resId)
}
...
fun getResourceId(name: String, type: String): Int {
    return MyApp.context.resources.getIdentifier(name, type, appPackage)
}
...
fun getApplicationMetaData(key: String): String? {
    ...
    try {
        applicationInfo = MyApp.context.packageManager.getApplicationInfo(
            appPackage,
            PackageManager.GET_META_DATA
        )
    } catch (e: PackageManager.NameNotFoundException) {
        logW("AppMetaData", e.message, e)
    }
...
fun isInstalled(packageName: String): Boolean {
    val packageInfo: PackageInfo? = try {
        MyApp.context.packageManager.getPackageInfo(packageName, 0)
    } catch (e: PackageManager.NameNotFoundException) {
        null
    }
